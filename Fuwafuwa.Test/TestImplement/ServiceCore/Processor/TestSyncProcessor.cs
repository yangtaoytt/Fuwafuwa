...
public class TestSyncProcessor<TSharedDataWrapper> : IProcessorCore<StringData,TSharedDataWrapper,object> 
    ...
    public async Task<List<Certificate>> ProcessData(StringData data, TSharedDataWrapper sharedData, Logger2Event? logger) {
        ...
        sharedData.Execute(refData => {
            ...
            refData.Value = true;
            logger?.Info(this, $"Processing thread:{Thread.CurrentThread.Name}:{Environment.CurrentManagedThreadId} level 0");
            Task.Delay(1000).Wait();
            logger?.Info(this, $"Processing thread:{Thread.CurrentThread.Name}:{Environment.CurrentManagedThreadId} level 1");
            Task.Delay(1000).Wait();
            logger?.Info(this, $"Processing thread:{Thread.CurrentThread.Name}:{Environment.CurrentManagedThreadId} level 2");
            Task.Delay(1000).Wait();
            refData.Value = false;
        });
        return [];
